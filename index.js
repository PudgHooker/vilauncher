...
$(document).ready(function () { verAndAccManager.showform(); });
...
$("#filtershide").mouseover
    (function () {
        if (select == 0) {

            $("#filtershide").css({ 'background-color': '#ff7e00' });
        }
        else {
            $("#filtershide").css({ 'background-color': '#e6e6e6' });
        }

    }
    );

$("#filtershide").mouseout

    (function () {
        if (select == 0) {

            $("#filtershide").css({ 'background-color': '#34b6e4' });
        }
        else {
            $("#filtershide").css({
...
$("#filtershide").click
    (function () {
        if (noanim == 3)
            return;
        if (select == 0) {
            $("#filterimage").css({ 'filter': 'invert(0)' });
            $("#sastext").css({ 'color': '#323232' });
            $("#filtershide").css({ 'background-color': '#ffffff' });
            ...
            $("#filtersme").animate({
                height: 0,
                    ...
                    complete: function () {
                        ...
                        $("#contentss").css({ 'visibility': 'hidden' });
                        noanim = 1;
                    },
                
                    start: function () { noanim = 3; }
                });
            ...
            $("#filterimage").css({ 'filter': 'invert(1)' });
            $("#sastext").css({ 'color': '#ffffff' });
            $("#filtershide").css({ 'background-color': '#34b6e4' });
            $("#contentss").css({ 'visibility': 'visible' });
...
function bindButtons() {
    ...
    bindButton($(".js-ms_favourite_button"), function (e, el) {
        $.when(uiLoader.show()).then(function () {
            setTimeout(
                function () {
...
function noserv()
{
    $("#noserv").removeClass('hidden');
}
function alotserv()
{
    $("#noserv").addClass('hidden')
}
...
const accManager = {
	...
	on_editdone: function() {
        ...
        verAndAccManager.deleteUser(this.delname);
...
const settingsManager = {
	...
	on_save: function() {
        ...
        verAndAccManager.enableSnap(settingsManager.getSnap());
        verAndAccManager.enableFull(settingsManager.getFull());
        verAndAccManager.enableReg(settingsManager.getReg());
        verAndAccManager.safecfg();
	},
    ...
    getSnap: function ()
    {
        return $(".js-ms_setmanager_fullscreen").prop("checked");
    },
    getFull: function ()
    {
        
        return $(".js-ms_setmanager_fullscreen2").prop("checked");
    },
    getReg: function ()
    {
        return $(".js-ms_setmanager_fullscreen3").prop("checked");
    },
...
const verManager = {
	...
	show: function() {
		...
		uiShadow.show();
        if (verAndAccManager.issnaps()) {
          
            $('#skritsnap').addClass('hidden');
        }
        else
        {
            $('#skritsnap').removeClass('hidden'); 
        }
	...
	set: function(obj) {
        ...
        for (var i = notinstalled.length - 1; i >= 0; i--)
            if (notinstalled[i].indexOf("snapshot") >= 0) {

                $('.js-ms_vermanager_notinstalled2').after(e = this._createItem(makeTitle(notinstalled[i]), notinstalled[i])), bindButton(e, function (foo, el) {
                    _this.on_click(el);
                });
            }
            else {
        ...
        function _createElem(title, value) {
            ...
            if (value[1].indexOf("-") >= 0) {
                e_title.innerHTML = title + ' ' + value[1].split('-')[0];

            } else {
                e_title.innerHTML = title + ' ' + value[1];
            }
		...
		function makeTitle(ver) {
            ...
            if (ver[0].indexOf("release") >= 0)
                return ver[0].replace("release", "Версия");
            if (ver[0].indexOf("snapshot") >= 0)
                return ver[0].replace("snapshot", "Snapshot");
			return ver[0];
		}
	},
	...
	on_click: function(el) {
        ...
        {
            verAndAccManager.aborted(this.getSelectedVer());
            $('#btn_vermanager__install').addClass('hidden')
	...
	_createItem: function(name,value) {
        ...
        if (value[1].indexOf("-") >= 0) {
            title.innerHTML = name + ' ' + value[1].split('-')[0];

        } else {


            title.innerHTML = name + ' ' + value[1];
        }
...
const blockFilters = {
    ...
    on_click: function (e) {
        ...
        if ($(e).hasClass('item--active')) {
            ...
            $('mainfilt').addClass('item--active');
            $.when(uiLoader.show()).then(function () {
                setTimeout(
                    function () {

                    }, 50);
                servManager.setmainfilter("");
                uiLoader.hide();
            });
...
const blockServers2 = {
    ...
    display: {
        ...
        // wh - где отгружать (where) [top,down]
        load: function (n, wh) {
            var IsBefore = 0, ej = $('#cnt-servers2').children(); //ej - element jquery
            if (wh == 'top')
                IsBefore = 1
                    , ej = ej.first()
        ...
        clear: function () {
            ...
            } $("#cnt-servers2").children(":not(.js-nodelete)").remove();
    ...
    on_likebutton: function (e, e_item) {
        ...
        blockServers.updateFav();
        $(e_item).removeClass('item--favourite');
        
    },
    on_playbutton: function (e, ver) {
        verAndAccManager.setServer(e.getAttribute('data-addr'));
        verAndAccManager.selectver(ver);
    ...
    _createItem: function (num, obj) {
        ...
        e_img.onerror = function () {
            ...
            e_img.className = 'item__icon2';
            e_img.src = 'resource/white.jpg';
...
const blockServers = {
    ...
    updateFav: function () {
        ...
        for (var i = 0; i < this.display.items.length; i++) {
            let obj = this.display.items[i];
            
            if (servManager.inFav(obj.getElementsByClassName('likebutton')[0].getAttribute('data-serverid')) == 0)
            {
                {
                    

                    $(obj).removeClass('item--favourite');
                }
            } 
        }
    },
    ...
    on_likebutton: function (e, e_item) {
        ...
        if (servManager.inFav(e.getAttribute('data-serverid')) == 1) {
            servManager.addToFav(e.getAttribute('data-serverid'));
            $(e_item).removeClass('item--favourite');
